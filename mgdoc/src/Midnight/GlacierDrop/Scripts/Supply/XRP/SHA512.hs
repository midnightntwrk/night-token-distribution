...
module Midnight.GlacierDrop.Scripts.Supply.XRP.SHA512 (sha512, sha512Half, sha512HalfBS) where
...
-- | Computes SHA-512Half hash (first 32 bytes of SHA-512)
sha512Half :: BuiltinByteString -> BuiltinByteString
sha512Half msg = sliceByteString 0 32 (sha512 msg)
{-# INLINEABLE sha512Half #-}

-- | Computes SHA-512Half hash (first 32 bytes of SHA-512) using the `hashWith` function, this cannot be used in onchain code.
-- This is for offchain equivalence testing with the plutus implementation `sha512Half`.
sha512HalfBS :: ByteString -> ByteString
sha512HalfBS = BS.take 32 . convert @_ @ByteString . hashWith SHA512
